...
class TitleState extends MusicBeatState
{
	...
	override function beatHit()
	{
		...
		// dogeDance.animation.play('dance', true);
		...
		{
			case 1:
				createCoolText(['ArfieCat', 'Blue', 'Odie', 'Wickedbelle', 'YungRhythm']);
			case 3:
				addMoreText('Doge Funkin Team');
			case 4:
				deleteCoolText();
			case 5:
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxTween.tween(logoBl,{y: -75}, 2, {ease: FlxEase.expoOut});

			// new FlxTimer().start(0.01, function(tmr:FlxTimer)
			// {
			// 	if(logoBl.angle == -4) 
			// 		FlxTween.angle(logoBl, logoBl.angle, 4, 4, {ease: FlxEase.quadInOut});
			// 	if (logoBl.angle == 4) 
			// 		FlxTween.angle(logoBl, logoBl.angle, -4, 4, {ease: FlxEase.quadInOut});
			// }, 0);
