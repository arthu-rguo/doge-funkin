		...
		if (!PlayStateChangeables.Optimize)
		{
			switch(stageCheck)
			{
				...
				case 'couch':
					...
					var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('couch/couchback'));
					bg.screenCenter();
					bg.x += 12;
					bg.y += 25;
					...
					var fg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('couch/couchfront'));
					fg.screenCenter();
					fg.x += 115;
					fg.y += 250;
					fg.setGraphicSize(Std.int(fg.width * 0.61));
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				{
					if (boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss') && boyfriend.animation.curAnim.curFrame >= 14 || boyfriend.animation.curAnim.finished)
						boyfriend.playAnim('idle');
				}
